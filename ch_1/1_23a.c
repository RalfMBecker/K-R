...
char matrix_quotes_double[MAXROW][MAXLINE]; 
/* to actually use this idea, would need to rewind to start. so won't work */
...
/* mark positions in text that are in and out of double quotes */
void build_mqd(char* m){
  ...
  for (i=0; i < MAXROW; ++i)
    for (j=0; j < MAXLINE; ++j)
      matrix_quotes_double[i][j] = OUT;

  row = -1;
  while ( (++row < MAXROW) && my_getline(line, MAXLINE) > 0){
    j=0;
    while ( (ct = line[j] != EOF) && ct != '\n'){
      if (ct == '\"')
	state = ++state%2; /* == ~ if state 0 or 1 */
      matrix_quotes_double[row][j] = state;
    }
  }
