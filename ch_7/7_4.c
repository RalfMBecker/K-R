...
int main(){
  ...
  printf("**Implements types d, i, o, x, e, f, g, c, s (c untested)\n");
...
int mysscanf(char* source, char* fmt, ...){
  ...
  for ( ; source[i] != '\0' && ( c = fmt[j] ) != '\0'; j++){
	...
	*pival = atoi(vstring);
    ...
    default:
      info[0] = c;
      printf("Error: conversion for type '%s' failed.\n", info);
      return -1;
...
// pardon the goto. just more structured here.
int pvalue(char* vstr, char* source, int from, 
	   char type, int number, int sep, char* pbase, int len){
