...
long filelength(char* name){
  ...
  if (stat(name, &stbuf) == -1){
    fprintf(stderr, "filelength: can't access size information of %s\n", name);
    return -1L;
  }
...
int myfflush(myFILE* stream){
  ...
  if (stream == NULL){  
    ...
    myfflush(fp);
    from = &_iob[3];
    to = _iob + OPEN_MAX-3; // code review Sep 11, 2013: should be OPEN_MAX???
  }
  else{
    from = stream;
    to = from + 1;
  ...
  for (fp = from; fp < to; fp++){
    // code review Sep 11, 2013: don't allow streams flagged as _WRITE | _READ?
    //                           seems better to treat as _WRITE stream?
...
int _flushbuf(char c, myFILE* fp){
  ...
  if (fp->base == NULL){ // start a new buffer
    ...
    // code review Sep 11, 2013: ??? Writing c back to itself, why?
